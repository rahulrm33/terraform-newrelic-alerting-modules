...
resource "newrelic_nrql_alert_condition" "nrql_alert_condition" {
  ...
  nrql {
    query = "SELECT filter(average(newrelic.timeslice.value), WHERE metricTimesliceName = 'HttpDispatcher') OR 0 FROM Metric WHERE appName IN ('${local.app_names_string}') AND metricTimesliceName IN ('HttpDispatcher', 'Agent/MetricsReported/count') FACET appId"
  }
...
# locals {
#   app_names_string = join("', '", var.app_names)
#   nrql_query_template = "SELECT filter(average(newrelic.timeslice.value), WHERE metricTimesliceName = 'HttpDispatcher') OR 0 FROM Metric WHERE appName IN ('${local.app_names_string}') AND metricTimesliceName IN ('HttpDispatcher', 'Agent/MetricsReported/count') FACET appId"
# }
...
#   nrql {
#     query = replace(local.nrql_query_template, "WHERE ", "WHERE appName IN ('${local.app_names_string}') AND ")
#   }
...
#   nrql {
#     query = <<-EOF
#     SELECT filter(average(newrelic.timeslice.value), WHERE metricTimesliceName = 'HttpDispatcher') OR 0 FROM Metric 
#     WHERE appName IN (${join(",", var.app_names)}) AND metricTimesliceName IN ('HttpDispatcher', 'Agent/MetricsReported/count') 
#     FACET appId
#     EOF
#   }
